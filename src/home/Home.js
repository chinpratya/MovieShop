...
export default function User() {
    ...
    useEffect(() => {
      return () => {
        axios.get("https://api.themoviedb.org/3/movie/popular?api_key=a4893bf47ad0104883bb0681284dc40e&language=en-US&page=1").then((res) => {
  ...
  return (
	...
	<div class="modal-dialog modal-confirm">
		<div class="modal-content">
			...
			<div class="modal-body">
				<img src={require("../assets/shopping-cart.png")} alt="" className='imgAlert'/>
			</div>
    ...
    {movies.map((data,index) =>(
          <div className="cardBX" key={data.id}>
                    <img src={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} alt="" />
                    <Link to={`/detail/${data.id}`} ><p className='title'>{data.original_title}</p></Link>  
                            <p className='price'>ราคา : {Math.ceil(data.vote_average*100)} บาท</p>
                        <div className="btbBx">
                            <button className='ntn-buy'  onClick={()=>AddCart(data)}>เพิ่มลงตะกร้า</button>
